...
export default function PreviewPane({ preview, compileState, projectId, projectError, onCompile, onReset }: Props) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#f5f5f7]">
      {/* Tab bar */}
      <div className="glass-bar flex items-center h-12 px-4 gap-2.5">
        <div className="inline-flex bg-black/5 rounded-full p-[3px]">
          {(['preview', 'compile'] as Tab[]).map(t => (
            <button
              key={t}
              onClick={() => setTab(t)}
              className="relative px-4 py-1.5 text-[12.5px] font-medium capitalize rounded-full transition-colors"
              style={{ color: tab === t ? '#1d1d1f' : '#6e6e73' }}
            >
              {tab === t && (
                <motion.div
              ...
              <span className="relative">{t === 'preview' ? 'Preview' : 'Compile'}</span>
        ...
        {tab === 'preview' && (
          <div className="ml-auto flex items-center gap-0.5">
            <button
              ...
              title="Zoom out"
              aria-label="Zoom out"
              className="p-1.5 rounded-md text-gray-400 hover:text-gray-700 hover:bg-black/5 disabled:opacity-30 disabled:hover:bg-transparent disabled:hover:text-gray-400 transition-colors"
            >
            ...
            <button
              ...
              className="min-w-[3rem] px-1 py-1 text-[11px] tabular-nums text-gray-500 hover:text-gray-900 rounded-md hover:bg-black/5 transition-colors"
            >
              {Math.round(zoom * 100)}%
            </button>
